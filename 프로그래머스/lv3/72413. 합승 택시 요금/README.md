...
### 풀이 과정
1. 플로이드 워셜을 사용하여 각 노드에서 다른 노드까지의 최소거리 표를 만든다.
2. S에서 A와 B까지 중첩이 있는 경우를 포함하여 최단거리는 어떠한 노드에서 A까지의 거리 B까지의 거리 S까지의 거리의 합이 최소인 경우이다.
3. 만들어진 표를 순회하며 해당 노드를 찾아 출력한다.
